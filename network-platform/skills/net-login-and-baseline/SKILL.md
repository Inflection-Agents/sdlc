...
**Executable form:** this skill is implemented by the [`netcli` connector](../../connector/README.md) — `baseline_device` / `baseline_devices` run the per-role sets over the topology-map inventory, `snapshot` saves a capture, and `diff` turns a before/after pair into a drift report that surfaces material change (config set-line deltas, downed BGP peers, route-count drops, new alarms, version changes) while suppressing volatile noise (counters, uptimes, timestamps). Next: structured parsing (NAPALM/Genie) so the diff compares typed fields where available.
