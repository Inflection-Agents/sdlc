...
| Criterion | Evidence |
| --- | --- |
| The gate terminates | ADR-004 + `sdlc-state-machine.yaml` exit condition; 9 doctrine sites carry the cap; `gen-handoffs --check` green |
...
| Expired documents leave the default search path | SPEC-005 archived; all four `rg` forms miss it; `resolve.mjs` and `--no-ignore` reach it |
