...
This supersedes three sites in ADR-003, each stating the same uncapped rule: the `Capped fix loop (≤3)` row of its capability table, the closing clause of Decision item 10, and the re-verification clause under the panel-composition decision. With them goes
...
recorded inline at all three sites instead, and named here.
