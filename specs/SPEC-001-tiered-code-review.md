...
### v1.3 (2026-09-09) — altitude routing surfaced via SPEC-007
- SPEC-007 adds a **Lens and altitude attribution** section to live `review-primitives.md`, transcribing the `lens` and `altitude` field semantics that already ship in `review-envelope.schema.json`. This is a reconciliation, not a new contract: the enum, the routing meaning, and the absent-implies-`implementation` default are unchanged from the schema. The defect was that `review-primitives.md` calls itself the operational contract for the output schema while omitting a field that schema defines, so a design-altitude finding had no documented route out of the fix loop.
- **Routing effect now stated in the contract:** a finding with `altitude: design` routes to `spec-amendment` or a replan rather than the bounded implementation fix loop. Severity grading is untouched — the severity spine and both consequence catalogs remain the sole grading inputs.
- Also records ADR-004's three-round cap on the integration gate, which bounds the fix loop this field routes into. A blocker or major surviving round 3 is disclosed in the integration PR body rather than carried into a fourth round.
- The extension pattern (SPEC-004) applies: SPEC-001 is `completed` and therefore closed to `spec-amendment`, so a subsequent spec extends the live artifact and records the change here.
